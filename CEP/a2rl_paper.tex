...
Hardware-in-the-loop experiments were conducted using an EAV24 formula race-car simulator. Across 1000 laps, the method achieved an overtaking success rate of 94.7\% and a mean overtaking time of 4.82 s. Compared with four baselines using the same execution stack, the success rate was 31.2–66.2 percentage points higher and the mean overtaking time was 2.43–6.41 s shorter; no collisions or track-boundary violations were observed.
...
% =====================================================================
\section{Game-Supervised Tactical Feasible-Set Policy}\label{sec:tactical}
% =====================================================================
...
\subsection{Executable Stackelberg Response-Set Supervision}
...
Table~\ref{tab:setup_params}, the guard order
...
\subsection{Persistent Tactical Modes and Corridor Carving}
...
\ELSIF{the locked ego satisfies $\Delta s_t\le-g_{\rm ot}$ for $N_{\rm post}$ updates}
...
synchronized \textsc{Shadow} and \textsc{Overtake} frames. Across the four
encounters, the paired views show how the feasible region evolves from
approach guidance to a committed passing channel. In Case~I, the
\textsc{Shadow} corridor at $\Delta s=18.6$~m places the planned path on the
selected side behind the opponent; at $\Delta s=0.1$~m, the
\textsc{Overtake} corridor excludes the opponent footprint and preserves a
continuous path along the straight. Case~II exhibits the same progression
through a tight bend: the corridor first establishes the lateral approach at
$\Delta s=17.8$~m and then follows the track curvature while maintaining the
locked-side channel at $\Delta s=0.0$~m. In Case~III, the corridor conforms to
the sweeping corner during \textsc{Shadow} and carries the planned path past
the opponent as the gap decreases from $15.0$ to $0.2$~m. Case~IV selects the
complementary side in the opposite curved geometry; its gradual
\textsc{Shadow} offset develops into a fully carved \textsc{Overtake} region
as $\Delta s$ decreases from $14.3$ to $0.1$~m. The eight spatial views thus
show that the state-dependent corridor consistently converts side intention
into a connected, track-conforming passing region across straight and curved
sections. Cases~I and II are then examined in detail as representative
high-speed overtakes on a straight and through a consecutive-corner sequence.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{track_overview_case0205_v019.png}
\caption{Representative overtaking situations on the Yas Marina circuit. }
\label{fig:four_case_track_overview}
\end{figure*}

\subsubsection*{Case I: high-speed overtake on a straight}

Case~I represents a high-speed overtake on the long straight. In
Fig.~\ref{fig:four_case_track_overview}, the \textsc{Shadow} view at
$\Delta s=18.6$~m shows the corridor forming a side-consistent approach channel
behind the opponent, with the planned path gradually displaced toward the
selected passing side. When the relative longitudinal gap decreases to
$0.1$~m, the \textsc{Overtake} carver excludes the opponent-occupied region and
retains a connected corridor on the locked side.
...
trajectory stays inside this corridor through the side-by-side passage.
Triangles and squares mark footprint-based Encounter and Pass events. For the
nearly aligned vehicles, Encounter begins at
$\Delta s_{\rm E}\simeq(L_e+L_o)/2=5.2$~m, when the longitudinal body envelopes
first overlap after the corridor has established $|\Delta n_{\rm E}|$. Pass is
the side-by-side instant $\Delta s_{\rm P}\simeq0$, where
$|\Delta n_{\rm P}|$ exceeds the combined projected half-width,
$(W_e+W_o)/2=1.9$~m. Thereafter, $\Delta s<0$ records the ego reference point
moving ahead, and $\Delta s\leq-5.2$~m marks full longitudinal clearance. The
actual speed follows the desired profile and exceeds 70~m~s$^{-1}$ during the
pass.
...
passing channel. Lateral contouring, speed-tracking, and heading errors remain
close to zero, apart from brief transients during the tactical transition.
All recorded tangential--normal acceleration samples remain inside the
speed-dependent tire envelope. Together, these results show that the
committed-side corridor is converted into a trackable path--speed reference
throughout the high-speed pass.
...
\subsubsection*{Case II: high-speed overtake through consecutive corners}

Case~II represents a high-speed overtake through successive curvature changes.
In Fig.~\ref{fig:four_case_track_overview}, the \textsc{Shadow} view at
$\Delta s=17.8$~m shows the corridor following the approaching bend and guiding
the planned path toward the selected lateral position. At $\Delta s=0.0$~m,
full \textsc{Overtake} exclusion preserves the same-side channel as the path
curves around the opponent within the track boundaries.
Figure~\ref{fig:case05_tactical_new} summarizes the corresponding FSM sequence,
Frenet trajectories, and speed response. In panel~(b), $s-s_0$ denotes
trackwise progress relative to the initial ego position, and $n$ denotes the
lateral position relative to the track centerline. The published feasible
corridor follows the changing track boundaries, and the actual ego trajectory
remains inside it while reversing lateral direction through the consecutive
corners. At Encounter, the curved corridor has already established
$|\Delta n_{\rm E}|$ as the longitudinal footprints begin to overlap. At Pass,
$\Delta s_{\rm P}\simeq0$ and $|\Delta n_{\rm P}|$ remains above the corresponding
projected-width threshold, preserving lateral clearance through the
side-by-side configuration.
From an entry speed above 60~m~s$^{-1}$, the actual speed follows the desired
braking and re-acceleration profile throughout the pass.
...
Figure~\ref{fig:case05_tracking_new} shows the closed-loop realization of this
curved passing channel. Lateral contouring, speed-tracking, and heading errors
remain bounded about zero, with short transients during the strongest curvature
and speed transitions. All recorded tangential--normal acceleration samples
remain inside the speed-dependent tire envelope over the recorded speed range.
Together, these results show that the side-locked corridor is converted into a
trackable path--speed reference despite successive changes in curvature and
...
guidance on closed-loop overtaking performance.
...
\begingroup
\scriptsize
\emph{Notes:} Prop., proposed method; NI-MPC, Non-Interactive MPC;
RB-GT, Rule-based GT; iLQG, iLQGames. TTO is averaged over successful
overtaking attempts. Min. gap is the mean per-run minimum net clearance
between the vehicle body envelopes. PFR is the fraction of planning cycles
with a feasible SOCP solution. Bold denotes the best observed value; equal
track-violation values are jointly bold.
\par\endgroup
\end{table}
...
\midrule
Nominal & $94.7$ & $4.82$ & $5.83$ & $0.0$ & $1.000$ \\
