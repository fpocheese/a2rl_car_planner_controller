...
with the mode-dependent opponent exclusion.  If the raw interval
...
For the MPCC, define $\bm\chi_k=[x_k,y_k,\theta_k]^\top$ and
$\bm\nu_k=[V_k,\delta_k]^\top$. Its identified local prediction model and the
...
\STATE map the first acceleration/steering command to throttle, brake,
