...
/* ========== 全局安全参数 ========== */
static constexpr double kDsafe = 1.7;  // 赛道边界安全裕度 (m)，半车宽
...
OptPlanner::OptPlanner(int input_point_num, double input_point_dt) : out_point_num(input_point_num), out_point_dt(input_point_dt) {
    ...
    ggv_exp = 0.75;           // GGV 超椭圆形状指数，默认 2.0 (完美椭圆)

    // 外部可行域覆盖，默认关闭
    corridor_override_enable = false;
    corridor_N = 0;
    // 终端目标模式: 0=raceline优先(仅在可行域外才钳位到中心), 1=始终使用可行域中线
    corridor_terminal_mode = 1;
...
void OptPlanner::setCorridorOverride(const std::vector<double> &S,
                                     const std::vector<double> &Lmax,
                                     const std::vector<double> &Lmin,
                                     int tactical_mode) {
    if (tactical_mode == 0) {
        // race模式：不使用外部可行域，走旧的障碍物 Dsafe 修正逻辑
        corridor_override_enable = false;
        corridor_terminal_mode = 0;
        // printf("Corridor Override DISABLED (race mode): tactical=%d, using Dsafe obstacle logic\n",
        //        tactical_mode);
        return;
    }

    // 非race模式：使用外部可行域覆盖
    corridor_N = static_cast<int>(S.size());
    corridor_S = S;
    corridor_Lmax = Lmax;
    corridor_Lmin = Lmin;
    corridor_override_enable = true;
    corridor_terminal_mode = 1;  // 非race模式 → 使用可行域中线作为终端目标

    // printf("Corridor Override Enabled: N=%d, S=[%.1f, %.1f], tactical=%d (terminal_mode=%d)\n",
    //        corridor_N, corridor_S.front(), corridor_S.back(), tactical_mode, corridor_terminal_mode);
}

void OptPlanner::clearCorridorOverride() {
    corridor_override_enable = false;
    corridor_N = 0;
    corridor_S.clear();
    corridor_Lmax.clear();
    corridor_Lmin.clear();
    // printf("Corridor Override Disabled.\n");
}

int OptPlanner::getCorridorSRange(double meX, double meY, double meAngle,
                                  double &s_start, double &s_end, double &lap_length) {
    return getCorridorSRange(meX, meY, std::numeric_limits<double>::quiet_NaN(),
                             meAngle, s_start, s_end, lap_length);
}

int OptPlanner::getCorridorSRange(double meX, double meY, double meZ, double meAngle,
                                  double &s_start, double &s_end, double &lap_length) {
    if (!is_init) {
        printf("OptPlanner is not initialized.\n");
        return -1;
    }

    // 临时 STATE 做坐标变换，获取 ego 的 s 坐标
    auto tempState = std::make_shared<STATE>();
    tempState->N = 1;
    tempState->s = std::vector<double>(1);
    tempState->l = std::vector<double>(1);
    tempState->x = std::vector<double>(1, meX);
    tempState->y = std::vector<double>(1, meY);
    if (std::isfinite(meZ)) {
        tempState->z = std::vector<double>(1, meZ);
    }
    tempState->a = std::vector<double>(1, meAngle);
    tempState->da = std::vector<double>(1);
    tempState->v = std::vector<double>(1);

    StateTrans(tempState, RaceLine, meZ);

    double dp = 300.0;
    lap_length = RaceLine->Sref[RaceLine->N - 1];

    // 返回取模后的值，都在 [0, lap_length) 范围内
    // 如果 s_end < s_start，说明跨圈了，内部会自动处理
    s_start = fmod(tempState->s[0], lap_length);
    s_end = fmod(tempState->s[0] + dp, lap_length);
...
int OptPlanner::WhereAmI(double xpos, double ypos, double angle) {
    ...
    p1 = 140 - 1;   // 2505 - 1   suzuka:2852 - 1   northold 1377   AD:2890
    // p2 = PitLine1->N;
    p2 = 300;
    for (i = p1; i < p2; i++) {
    ...
    // if (flag == 0) {
    //     p1 = 1 - 1;
    //     p2 = 300;  // 325    suzuka:182   northold 337  380会误检测   AD:120
    //     for (i = p1; i < p2; i++) {
    //         if (sqrt(pow(xpos - PitLine1->Xref[i], 2) + pow(ypos - PitLine1->Yref[i], 2)) <= Rdec) {
    //             if (fabs(wrapToPi(angle - wrapToPi(PitLine1->Aref[i]))) < Adec) {
    //                 flag = 1;
...
std::tuple<double, double, double, double, double, double> OptPlanner::GetState(double meX, double meY, double meAngleOffset, double meSpeed) {
    return GetState(meX, meY, std::numeric_limits<double>::quiet_NaN(), meAngleOffset, meSpeed);
}

std::tuple<double, double, double, double, double, double> OptPlanner::GetState(double meX, double meY, double meZ, double meAngleOffset, double meSpeed) {
    const bool use_height = std::isfinite(meZ);
    ...
    StateI->z = use_height ? std::vector<double>(StateI->N, meZ) : std::vector<double>();
    ...
    StateIPit1->z = use_height ? std::vector<double>(StateIPit1->N, meZ) : std::vector<double>();
    ...
    StateIPit2->z = use_height ? std::vector<double>(StateIPit2->N, meZ) : std::vector<double>();
    ...
    StateSouthI->z = use_height ? std::vector<double>(StateSouthI->N, meZ) : std::vector<double>();
    ...
    StateTrans(StateI, RaceLine, meZ);
    StateTrans(StateIPit1, PitLine1, meZ);
    StateTrans(StateIPit2, PitLine2, meZ);
    StateTrans(StateSouthI, SouthRaceLine, meZ);
...
void OptPlanner::checkRampVelocityDecrease(float ref_speed, float rc_speed, float &previous_speed, float &target_speed, float &previous_target_velocity, int iter, float step_period_, double acceleration_ramp_g ) {
    ...
    An_ref = 1.0 * Interp1(Vehicle->V, Vehicle->An, Vehicle->N, previous_speed);
	Aw_ref = 1.0 * Interp1(Vehicle->V, Vehicle->Aw, Vehicle->N, previous_speed);
    ...
    if (rc_speed < ref_speed) {
        ...
        previous_speed = target_speed;  // FIX: else分支也必须更新previous_speed，否则经过慢弯后ramp从旧高值恢复导致超速
...
int OptPlanner::StartLocalOptimize(int flag_race_left_right, double meX, double meY, double meAngleOffset, double meSpeed, std::vector<double> othersX, std::vector<double> othersY, std::vector<double> othersAngleOffset, std::vector<double> othersSpeed) {
    return StartLocalOptimize(flag_race_left_right, meX, meY,
                              std::numeric_limits<double>::quiet_NaN(),
                              meAngleOffset, meSpeed, othersX, othersY,
                              othersAngleOffset, othersSpeed);
}

int OptPlanner::StartLocalOptimize(int flag_race_left_right, double meX, double meY, double meZ, double meAngleOffset, double meSpeed, std::vector<double> othersX, std::vector<double> othersY, std::vector<double> othersAngleOffset, std::vector<double> othersSpeed) {
    if (!is_init) {
...
int OptPlanner::TrajOptGlobal() {
    ...
    RaceLine->Dsafe = kDsafe;
    RaceLine->PTP_on = 0;
    RaceLine->Zref.assign(RaceLine->N, 0.0);
...
int OptPlanner::TrajOptLocal(int flag_race_left_right) {
    ...
    if (corridor_override_enable && corridor_N > 0) {
        /* ===== 模式A: 外部可行域覆盖 ===== */
        /* 跳过旧的障碍物扣除逻辑，直接用外部传入的可行域，
           只和原始赛道边界 (Select_RaceLine->Lmax/Lmin) 取交集。
           外部传入的 corridor_S 全部是取模后的值 [0, LapLength)，
           跨圈时 corridor_S 可能不单调（如 [5400,...,5499,0,...,200]），
           内部自动处理。 */

        // 检测是否跨圈：如果 S 数组中存在后一个值 < 前一个值，则跨圈
        bool is_wrap = false;
        int wrap_idx = -1;
        for (int k = 1; k < corridor_N; k++) {
            if (corridor_S[k] < corridor_S[k - 1]) {
                is_wrap = true;
                wrap_idx = k;  // 跨圈断点索引
                break;
            }
        ...
        for (i = 0; i < N; i++) {
            double Si = Select_RaceLine->Sref[i];
            double ext_lmax_val, ext_lmin_val;
            bool found = false;

            if (!is_wrap) {
                // 不跨圈：corridor_S 单调递增，直接插值
                if (Si >= corridor_S.front() && Si <= corridor_S.back()) {
                    ext_lmax_val = Interp1(corridor_S, corridor_Lmax, corridor_N, Si);
                    ext_lmin_val = Interp1(corridor_S, corridor_Lmin, corridor_N, Si);
                    found = true;
                }
            } else {
                // 跨圈：corridor_S 被分为两段
                // 段1: corridor_S[0..wrap_idx-1] (圈尾部分)
                // 段2: corridor_S[wrap_idx..N-1] (圈首部分)
                int n1 = wrap_idx;
                int n2 = corridor_N - wrap_idx;

                if (n1 > 0 && Si >= corridor_S[0] && Si <= corridor_S[wrap_idx - 1]) {
                    // 匹配段1
                    ext_lmax_val = Interp1(
                        const_cast<std::vector<double>&>(corridor_S),
                        const_cast<std::vector<double>&>(corridor_Lmax),
                        n1, Si);
                    ext_lmin_val = Interp1(
                        const_cast<std::vector<double>&>(corridor_S),
                        const_cast<std::vector<double>&>(corridor_Lmin),
                        n1, Si);
                    found = true;
                } else if (n2 > 0 && Si >= corridor_S[wrap_idx] && Si <= corridor_S[corridor_N - 1]) {
                    // 匹配段2：需要偏移指针
                    std::vector<double> s2(corridor_S.begin() + wrap_idx, corridor_S.end());
                    std::vector<double> lmax2(corridor_Lmax.begin() + wrap_idx, corridor_Lmax.end());
                    std::vector<double> lmin2(corridor_Lmin.begin() + wrap_idx, corridor_Lmin.end());
                    ext_lmax_val = Interp1(s2, lmax2, n2, Si);
                    ext_lmin_val = Interp1(s2, lmin2, n2, Si);
                    found = true;
                }
            }
            ...
            if (found) {
                // 和原始赛道边界取交集，确保不超出赛道
                if (ext_lmax_val < Lmax[i]) Lmax[i] = ext_lmax_val;
                if (ext_lmin_val > Lmin[i]) Lmin[i] = ext_lmin_val;
            }
        }
    } else {
        /* ===== 模式B: race模式，仅用赛道边界 + Dsafe 内缩 ===== */
        for (i = 0; i < N; i++) {
            Lmax[i] = Select_RaceLine->Lmax[i] - Select_RaceLine->Dsafe;
            Lmin[i] = Select_RaceLine->Lmin[i] + Select_RaceLine->Dsafe;
        }
    }
    ...
    /* 当外部可行域启用时，将终端目标钳位到终端点处的可行域内部 */
    if (corridor_override_enable && corridor_N > 0) {
        double lmax_end = PassLine->Lmax[N - 1];
        double lmin_end = PassLine->Lmin[N - 1];
        double l_center = (lmax_end + lmin_end) / 2.0;

        if (corridor_terminal_mode == 1) {
            // 模式1: 始终使用可行域中线作为终端目标
            StateF->l[0] = l_center;
        } else {
            // 模式0: raceline优先，仅在可行域外才钳位到中心
            if (StateF->l[0] > lmax_end || StateF->l[0] < lmin_end) {
                StateF->l[0] = l_center;
            }
        }
        // 航向偏差设为 0，不强制跟踪 raceline 航向
        StateF->da[0] = 0.0;
    }
...
int OptPlanner::PathOptLocal(const std::shared_ptr<STATE> StateF, int mode) {
    ...
    for (i = 0; i < N; i++) {
        ...
        Lmax[i] = (PassLine->Lmax[i]) / r_scale;
        Lmin[i] = (PassLine->Lmin[i]) / r_scale;
        lk[i] = 0.0;
        ak[i] = 0.0;
    }
...
int OptPlanner::VelOptForwardBackward(const std::shared_ptr<STATE> StateF, int mode) {
    ...
    /* ========== FB 加速度约束修正参数 ========== */
    /* 对查表得到的加速度极限做 offset + scale 修正：
     *   a_corrected = ay_offset + ay_scale * Interp1(...)
     * 这些参数统一应用于 V_limit、前向积分、后向积分三处 */
    double ay_offset = -1.0;    // 侧向加速度偏移 (m/s^2)
    double ay_scale  = 1.0;    // 侧向加速度缩放
    double ax_offset = 0.0;    // 纵向加速度(轮胎)偏移 (m/s^2)
    double ax_scale  = 1.0;    // 纵向加速度(轮胎)缩放
    double ae_offset = 0.0;    // 发动机加速度偏移 (m/s^2)
    double ae_scale  = 1.2;    // 发动机加速度缩放

    if (StateI->s[0] >= 450 && StateI->s[0] < 780) {
        ay_offset -= 2.2; 
        ax_offset -= 0.3;
    }
    if (StateI->s[0] >= 780 && StateI->s[0] <= 950) {
        ay_offset -= 1.5;  
    }
    if (StateI->s[0] >= 2400 && StateI->s[0] < 2650) {
        ay_offset -= 1.8; 
    }
    if (StateI->s[0] >= 2650 && StateI->s[0] <= 2800) {
        ay_offset -= 2.0;  
    }
    ...
    for (int i = 0; i < N; i++) {
        if (kappa[i] < 1e-8) {
            ...
            for (int iter = 0; iter < 20; iter++) {
                ...
                double ay_max = ay_offset + ay_scale * Interp1(Vehicle->V, Vehicle->An, Vehicle->N, vt);
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        double ax_max_tyre = ax_offset + ax_scale * Interp1(Vehicle->V, Vehicle->Aw, Vehicle->N, vt);
        double ax_max_engine;
        if (PassLine->PTP_on == 1) {
            ax_max_engine = ae_offset + ae_scale * Interp1(Vehicle->V, Vehicle->Ae1, Vehicle->N, vt);
        } else {
            ax_max_engine =  ae_offset + ae_scale * Interp1(Vehicle->V, Vehicle->Ae0, Vehicle->N, vt);
        }
        ...
        // }
        drag_decel = (-1.04765 * v_cur * v_cur + 47.5 * v_cur - 1609.73) / mass ;
    ...
    for (int i = N - 2; i >= 0; i--) {
        ...
        double ay_max = ay_offset + ay_scale * Interp1(Vehicle->V, Vehicle->An, Vehicle->N, vt);
        if (ay_max < 1e-3) ay_max = 1e-3;
        double ax_max_brake = ax_offset + ax_scale * Interp1(Vehicle->V, Vehicle->Aw, Vehicle->N, vt);
        ...
        // }
        drag_assist = (-1.04765 * v_next * v_next + 47.5 * v_next - 1609.73) / mass ;
...
void OptPlanner::readFiles(int lenBaseline, const std::string BaseLineFilename, int lenCarData, const std::string CarDataFilename,
    ...
    SouthRaceLineLeft->Dsafe = kDsafe;
    ...
    SouthRaceLineRight->Dsafe = kDsafe;
...
int OptPlanner::ReadBaseLine(const std::string &filename, std::shared_ptr<TRACKdata> BaseLine) {
    ...
    // 读取第一行，跳过标题
    std::getline(file, line);
    const bool has_zref = line.find("Zref") != std::string::npos;
...
int OptPlanner::ReadRaceLine(const std::string &filename, std::shared_ptr<TRAJdata> RaceLine) {
    ...
    while (std::getline(file, line) && count < RaceLine->N) {
        ...
        if (has_zref) {
            std::getline(iss, token, ',');
            RaceLine->Zref[count] = std::stod(token);
        }
...
void OptPlanner::StateTrans(const std::shared_ptr<STATE> State, std::shared_ptr<TRAJdata> RaceLine) {
    StateTrans(State, RaceLine, std::numeric_limits<double>::quiet_NaN());
}

void OptPlanner::StateTrans(const std::shared_ptr<STATE> State, std::shared_ptr<TRAJdata> RaceLine, double vehicle_z) {
    int i, j, idx = 0, st_len;
    double err, err_min, vx, vy, wx, wy, x1, x2, y1, y2;
    double st[101] = {0.0}, xt[101] = {0.0}, yt[101] = {0.0}, zt[101] = {0.0}, at[101] = {0.0};

    for (j = 0; j < State->N; j++) {
        double state_z = vehicle_z;
        if (!std::isfinite(state_z) && j < static_cast<int>(State->z.size())) {
            state_z = State->z[j];
        }
        const bool use_height = std::isfinite(state_z) && RaceLine->Zref.size() == static_cast<size_t>(RaceLine->N);

        /* Find the Nearest WayPoint */
        err_min = INFINITY;
        for (i = 0; i < RaceLine->N - 1; i++) {
            const double dx = State->x[j] - RaceLine->Xref[i];
            const double dy = State->y[j] - RaceLine->Yref[i];
            err = dx * dx + dy * dy;
            if (use_height) {
                const double dz = state_z - RaceLine->Zref[i];
                err += dz * dz;
            }
            if (err < err_min) {
                err_min = err;
        ...
        for (i = 0; i < st_len; i++) {
            ...
            if (use_height) {
                zt[i] = Interp1(RaceLine->Sref, RaceLine->Zref, RaceLine->N, st[i]);
            }
        ...
        err_min = INFINITY;
        for (i = 0; i < st_len; i++) {
            const double dx = State->x[j] - xt[i];
            const double dy = State->y[j] - yt[i];
            err = dx * dx + dy * dy;
            if (use_height) {
                const double dz = state_z - zt[i];
                err += dz * dz;
            }
            if (err < err_min) {
                err_min = err;
