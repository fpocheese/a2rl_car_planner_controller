...
\subsection{Scenario I: High-Speed Straight Overtake}
\label{subsec:case2}
The first scenario takes place on a high-speed straight section
($\kappa\approx0$, minimum radius $>900\,\mathrm{m}$) where both vehicles
operate above $68\,\mathrm{m/s}$.  The ego vehicle enters the
...
\subsection{Scenario II: Low-Speed Sharp-Corner Overtake}
\label{subsec:case5}
The second scenario takes place in a tight cornering section with a minimum
radius of $10\,\mathrm{m}$ ($\kappa_{\max}=0.098\,\mathrm{m^{-1}}$) and
speed variation from $12.5$ to $65.8\,\mathrm{m/s}$.  The ego vehicle
must remain side-by-side with the opponent through a high-curvature sequence
where the GGV envelope severely limits available acceleration.  The run
completes the pass with
...
\subsection{Scenario III: Mixed Straight-Corner Same-Speed Overtake}
\label{subsec:case4}
The third scenario tests the most demanding interaction geometry: an
equal-speed opponent on a mixed track section spanning $1.5\,\mathrm{km}$
of straights and corners ($\kappa_{\max}=0.037\,\mathrm{m^{-1}}$,
minimum radius $27\,\mathrm{m}$).  The ego vehicle must generate
its own speed advantage through corridor exploitation rather than relying on
a pre-existing closure-speed gap.  The analysis focuses on the decisive
$29\,\mathrm{s}$ engagement window during which the ego vehicle identifies,
commits to, and executes the overtake.  The run completes the pass with
...
The Shadow phase lasts $5.0\,\mathrm{s}$, during which the corridor narrows
toward the committed side; the Overtake execution then requires
$8.8\,\mathrm{s}$ to clear the opponent and transition to Hold.
...
$0.19\,\mathrm{m}$, larger than the two fast-closure cases but still well
...
\textsc{Shadow} through \textsc{Overtake} to \textsc{Hold}, with the mode
commitment sustained throughout the decisive interval.  The pass completion
with only $0.12\,\mathrm{m}$ of longitudinal margin confirms that the ego
vehicle generates just enough advantage through cornering speed
exploitation---a characteristic signature of same-speed adversarial racing
where large surpluses are unavailable.

\subsection{Simulation Summary}
\label{subsec:sim_summary}
The three scenarios are designed to cover complementary operating regimes that
a competitive racing system must handle:
\begin{itemize}
\item \textbf{Scenario~I (High-Speed Straight):} Tests whether the hierarchy
  can exploit a short high-speed closure window where decision latency directly
  determines success or failure.
\item \textbf{Scenario~II (Sharp-Corner Low-Speed):} Tests whether the
  corridor remains feasible through severe curvature where the GGV envelope
  restricts available acceleration and lateral space is tightest.
\item \textbf{Scenario~III (Mixed Straight-Corner, Same-Speed):} Tests
  the full adversarial capability against an equal-speed opponent where the
  ego must generate its own advantage from track-geometry exploitation.
\end{itemize}
